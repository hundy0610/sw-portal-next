...
export async function POST(req: NextRequest) {
  try {
    const { id, title, contentType, body, updatedBy } = await req.json() as {
      id?: string; title?: string; contentType?: "html" | "url"; body?: string; updatedBy?: string;
    };
    ...
    const manual = await saveManual({ id, title, contentType, body, updatedBy: updatedBy || "" });
