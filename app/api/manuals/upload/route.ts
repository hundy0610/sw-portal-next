...
// 첨부 원본은 Vercel Blob 이다. 4MB 는 Blob 의 한계가 아니라 Vercel 서버리스 요청 본문
// 한도(4.5MB) 때문이다 — 그보다 큰 파일은 사내 드라이브에 올리고 공유 링크를
// externalFileUrl 로 등록한다.
export async function POST(req: NextRequest) {
  ...
  try {
    const blob = await put(`manuals/${file.name}`, file, {
      access: "public",
