...
async function readNotionError(res: Response, step: string): Promise<string> {
  ...
  console.error(
    `[manuals/upload] ${step} 실패 status=${res.status} cf-ray=${res.headers.get("cf-ray")} server=${res.headers.get("server")} content-type=${res.headers.get("content-type")} body=${trimmed.slice(0, 500)}`,
  );
