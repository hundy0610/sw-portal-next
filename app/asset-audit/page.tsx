...
// 참여 동의 이전에 취지를 충분히 설명해야 하므로, 클릭해야 펼쳐지는 칩이 아니라
// 처음부터 전체 설명이 보이는 목록으로 표시한다.
    ...
    <div className="min-h-screen" style={{ background: C.bgPage }}>
      ...
      <div className="px-4 sm:px-6 md:px-10 py-10 md:py-14">
        <div className="max-w-2xl mx-auto">
          ...
          {!loading && cfg?.open && (
            <>
              {/* ── 왜 자산실사를 진행하나요 — 참여 동의 이전에 취지를 충분히 설명 ── */}
              <div className="mb-6">
                <p className="text-center mb-3" style={{ ...T.label, color: C.text2 }}>자산실사를 진행하는 이유</p>
                <ul className="space-y-2.5">
                  {PURPOSE_ITEMS.map(item => (
                    <li key={item.title} className="flex items-start gap-3 bg-white rounded-2xl p-3.5" style={{ border: `1px solid ${C.border}` }}>
                      <span className="shrink-0 w-8 h-8 rounded-full flex items-center justify-center" style={{ background: C.brandSoft, color: C.brand }}>
                        {item.icon}
                      </span>
                      <div className="min-w-0 pt-0.5">
                        <p style={{ ...T.label, color: C.text1 }}>{item.title}</p>
                        <p className="mt-0.5" style={{ ...T.body, ...pretty, color: C.text3 }}>{item.desc}</p>
                      </div>
                    </li>
                  ))}
                </ul>
              ...
              <p className="text-center mt-8" style={{ ...T.caption, ...pretty, color: C.text3 }}>
