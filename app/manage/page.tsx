...
function BugReportPanel() {
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 800, color: C.text1, margin: "0 0 4px" }}>버그리포트</h2>
          <p style={{ fontSize: 13, color: C.text3, margin: 0 }}>총 {filtered.length}건</p>
        </div>
        <button onClick={load} style={{ padding: "8px 14px", borderRadius: 8, background: C.primarySoft, color: C.primary, border: "none", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
          새로고침
        </button>
      </div>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" as const }}>
        <select value={filterPage} onChange={e => setFilterPage(e.target.value)}
          style={{ padding: "7px 12px", border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 13, color: C.text2, background: "#fff" }}>
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: 60, color: C.text4 }}>불러오는 중...</div>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: 60, color: C.text4 }}>리포트가 없습니다.</div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column" as const, gap: 12 }}>
          {filtered.map(r => {
            const sc = STATUS_COLOR[r.status] ?? STATUS_COLOR["접수됨"];
            return (
              <div key={r.id} style={{ background: "#fff", border: `1px solid ${C.border}`, borderRadius: 12, padding: "16px 20px" }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    {/* 메타 */}
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" as const, marginBottom: 8, alignItems: "center" }}>
                      <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 20, background: "#F1F5F9", color: C.text2 }}>{r.page}</span>
                      <span style={{ fontSize: 11, color: C.text3 }}>›</span>
                      <span style={{ fontSize: 11, color: C.text3 }}>{r.feature}</span>
                    ...
                    {/* 설명 */}
                    <p style={{ fontSize: 14, color: C.text1, margin: "0 0 10px", lineHeight: 1.6, wordBreak: "break-all" as const }}>{r.description}</p>
                    {/* 스크린샷 */}
                    {r.screenshotUrl && (
                      <button onClick={() => setPreview(r.screenshotUrl!)}
                        style={{ fontSize: 12, color: C.primary, background: "none", border: "none", cursor: "pointer", padding: 0, marginBottom: 8 }}>
                        📎 스크린샷 보기
                      </button>
                    )}
                    {/* 제출 정보 */}
                  ...
                  <div style={{ display: "flex", flexDirection: "column" as const, gap: 6, flexShrink: 0 }}>
                    ...
                    <button onClick={() => handleDelete(r.id)}
                      style={{ padding: "5px 12px", borderRadius: 20, border: `1px solid ${C.border}`, background: "#fff", color: C.text3, fontSize: 12, cursor: "pointer" }}>
                      삭제
                    </button>
                  </div>
                </div>
