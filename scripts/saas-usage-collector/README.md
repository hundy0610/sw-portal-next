...
> **⚠️ 이 스크립트의 독립 배포(GPO + 작업 스케줄러 + 상시 실행)는 폐기됐다.** 직원
> 동의 없이 상시 백그라운드로 도는 것이 부담이 커, 이미 동의받는 PC 자산실사(WPF,
> `windows_recreate`)에 통합해 반기 1회 스냅샷으로만 수집하는 쪽으로 방향을 바꿨다
> (개발요청서 요청 5 참고). 이 스크립트는 이제 **windows_recreate 개발자가 History
> 읽기 로직을 WPF/.NET으로 옮길 때 참고할 코드 레퍼런스** 용도로만 남겨둔다 — 아래
> 배포 안내(GPO/작업 스케줄러 섹션)는 더 이상 따르지 않는다.

설치형 SW가 아니라 브라우저(Chrome/Edge)로 접속해서 쓰는 웹 기반 SaaS를 추적하기 위한
독립 수집 스크립트였다. 기존 PC 자산실사 에이전트(WPF, `windows_recreate`)와는 완전히
무관하게 동작한다 — 소스 접근 권한이 없는 별도 저장소라 그쪽에 손댈 수 없어서, 작업
스케줄러로 배포하는 별도 스크립트로 만들었었다.
