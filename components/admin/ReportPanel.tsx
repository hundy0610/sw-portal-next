...
const isDaewong = (r: SubRow) => r.billingType === "대웅";
...
// ─── 대웅청구 배지 ────────────────────────────────────────────────────────
function DaewongBadge() {
  return (
    <span className="ml-1 inline-flex items-center rounded-full px-1.5 py-0.5 text-[9px] font-bold bg-orange-100 text-orange-600 ring-1 ring-orange-200">
      대웅청구
    </span>
  );
}
...
function DeptRowUnified({
  ...
  const daewongKrwConverted = rows.filter(isDaewong).reduce((s, r) => s + convertedKrw(r.annualKrw, r.annualUsd, rate, mode), 0);
  const netKrwConverted = totalKrwConverted - daewongKrwConverted;
  const hasDaewong = rows.some(isDaewong);
                ...
                <span key={cat} className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold ${chipCls}`}>
                  ...
                  {[...swMap.entries()].map(([sw, info]) => (
                    <span key={sw} className="inline-flex items-center gap-0.5">
                      ...
                      {info.billingType === "대웅" && <span className="text-[8px] font-bold text-orange-500">(대웅)</span>}
                    </span>
                  ))}
                </span>
              );
          ...
          <div>
            ...
            {hasDaewong && (
              <div className="text-[11px] text-orange-500 mt-0.5">
                대웅 {fmt(daewongKrwConverted)}원 포함
              </div>
            )}
          </div>
        </td>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {rows.map(r => {
                    ...
                    const daewong = isDaewong(r);
                    return (
                      <tr key={r.id} className={`border-t border-indigo-100 ${daewong ? "bg-orange-50/60" : "bg-white/70"} hover:bg-indigo-100/50`}>
                        <td className="px-6 py-2 text-indigo-700 truncate max-w-[140px]">{r.user || "—"}</td>
                        <td className="px-4 py-2 font-medium text-indigo-900">
                          {r.swName}
                          {daewong && <DaewongBadge />}
                        </td>
...
function buildView(
  ...
  const daewongTotal = filtered.filter(isDaewong).reduce((s, r) => s + convertedKrw(r.annualKrw, r.annualUsd, rate, mode), 0);
  const netTotal     = grandTotal - daewongTotal;
  const hasDaewong   = filtered.some(isDaewong);

  return { coMap, grandTotal, daewongTotal, netTotal, hasDaewong, totalLicenses: filtered.length };
}
...
export default function ReportPanel() {
  ...
  const { coMap, grandTotal, daewongTotal, netTotal, hasDaewong, totalLicenses } = buildView(
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="mt-4 flex flex-wrap gap-3 items-end">
          ...
          {hasDaewong && (
            <>
              ...
              <div>
                <p className="text-xs text-orange-500 font-medium">대웅 청구분</p>
                <p className="text-xl font-bold text-orange-600">{fmt(daewongTotal)}<span className="text-sm ml-1">원</span></p>
              </div>
              <div className="pb-1 text-amber-400 font-bold text-lg">=</div>
              <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <KpiCard label="구독 SW 종류" value={`${data.filters.categories.length}개`} sub="카테고리" />
        <KpiCard label="총 라이선스" value={`${totalLicenses}개`} sub="사용 중" />
        <KpiCard label={`${periodLabel} USD 합계`}
          value={`$${data.rows.reduce((s,r) => s + periodUsd(r.annualUsd, mode), 0).toFixed(0)}`}
          sub={periodSuffix} />
        <KpiCard label={`${periodLabel} KRW 합계`}
          value={`₩${fmt(data.rows.reduce((s,r) => s + periodKrw(r.annualKrw, mode), 0))}`}
          sub={periodSuffix} />
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {(filterCompany || filterDept || filterCat) && (
          <button onClick={() => { setFilterCompany(""); setFilterDept(""); setFilterCat(""); }}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {[...coMap.entries()].map(([co, deptMap]) => {
              ...
              const coDaewong     = coRows.filter(isDaewong).reduce((s, r) => s + convertedKrw(r.annualKrw, r.annualUsd, rate, mode), 0);
              const coNet         = coTotal - coDaewong;
              const coHasDaewong  = coRows.some(isDaewong);
                    ...
                    <td className="px-4 py-2 text-right">
                      ...
                      {coHasDaewong && (
                        <div className="text-[10px] text-orange-500 mt-0.5">
                          대웅 {fmt(coDaewong)}원 | 실부담 {fmt(coNet)}원
                        </div>
                      )}
                    </td>
                  </tr>
              ...
              <td className="px-4 py-3 text-right">
                ...
                {hasDaewong && (
                  <div className="text-[11px] mt-1 space-y-0.5">
                    <div className="text-orange-300">대웅 청구 {fmt(daewongTotal)}원</div>
      ...
      <p className="text-xs text-indigo-300 text-right">
        * 원화환산 기준: $1 = ₩{fmt(rate)} (실시간 환율) · 데이터 출처: Notion SW DB
        {hasDaewong && " · 대웅청구 항목은 법인 실부담에서 제외됩니다"}
      </p>
    </div>
  );
}
