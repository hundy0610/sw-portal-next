...
// ── SW 매크로 카테고리 매핑 ────────────────────────────────────────────
const SW_CAT_RULES: { label: string; icon: string; color: string; bg: string; keywords: string[] }[] = [
  {
  ...
  },
  {
    ...
    keywords: ["vscode", "vs code", "intellij", "pycharm", "eclipse", "xcode", "git", "github", "gitlab", "docker", "postman", "dbeaver", "sourcetree", "jira", "datagrip", "rider", "goland", "clion", "webstorm", "터미널", "terminal"],
  },
  {
    label: "협업 툴",   icon: "🤝", color: "text-orange-700", bg: "bg-orange-50",
    keywords: ["notion", "slack", "teams", "zoom", "webex", "google meet", "trello", "asana", "monday", "카카오워크", "miro", "confluence", "figma", "jira"],
  },
  {
...
function getSwMacroCategory(swName: string): { label: string; icon: string; color: string; bg: string } {
  if (!swName) return { label: "기타", icon: "📦", color: "text-gray-700", bg: "bg-gray-50" };
  const lower = swName.toLowerCase();
  for (const rule of SW_CAT_RULES) {
    if (rule.keywords.some(kw => lower.includes(kw))) {
      return { label: rule.label, icon: rule.icon, color: rule.color, bg: rule.bg };
    }
  }
  return { label: "기타", icon: "📦", color: "text-gray-700", bg: "bg-gray-50" };
}
...
function Pagination({ total, page, size, onChange }: {
  ...
  const btnBase = "w-8 h-8 rounded-lg text-xs flex items-center justify-center transition-colors";
  return (
    <div className="flex items-center justify-between mt-4">
      <span className="text-xs text-gray-400">{start}–{end} / {total}건</span>
      <div className="flex items-center gap-1">
        <button onClick={() => onChange(page - 1)} disabled={page === 1}
          className={`${btnBase} border border-gray-200 ${page === 1 ? "opacity-40 cursor-not-allowed" : "hover:bg-gray-100"}`}>‹</button>
        {pages.map((p, i) =>
          p === "…" ? (
            <span key={`e${i}`} className="w-8 text-center text-xs text-gray-400">…</span>
          ) : (
            <button key={p} onClick={() => onChange(p as number)}
              className={`${btnBase} ${page === p ? "bg-blue-600 text-white border-blue-600" : "border border-gray-200 hover:bg-gray-100 text-gray-600"}`}>
              {p}
            </button>
          )
        )}
        <button onClick={() => onChange(page + 1)} disabled={page === totalPages}
          className={`${btnBase} border border-gray-200 ${page === totalPages ? "opacity-40 cursor-not-allowed" : "hover:bg-gray-100"}`}>›</button>
      </div>
    </div>
  );
}
...
function CategoryView({ records }: { records: SwDbRecord[] }) {
  ...
  const catGroups = useMemo<CatGroup[]>(() => {
    // Step 1: group by macro category
    ...
    // Step 2: for each category, group by swCategory (SW name)
    const ORDER = ["문서작업용", "AI 툴", "개발 툴", "협업 툴", "디자인 툴", "보안/관리", "기타"];
    return ORDER.filter(label => catMap[label]).map(label => {
      const info = getSwMacroCategory(label === "기타" ? "" : label);
      ...
      const swGroups: SwGroup[] = Object.entries(swMap)
        .map(([swName, swRecs]) => {
          ...
          const days    = swRecs.map(r => daysLeft(r.renewalDate)).filter(d => d !== null && d >= 0) as number[];
  ...
  return (
    <div className="flex flex-col gap-5">
      {catGroups.map(cat => (
        <div key={cat.label}>
          {/* ── 카테고리 헤더 ── */}
          <div className={`flex items-center justify-between px-4 py-3 rounded-t-xl border border-b-0 ${cat.bg} border-current/20`}
            style={{ borderColor: "rgba(0,0,0,0.1)" }}>
            <div className="flex items-center gap-2">
              <span className="text-xl">{cat.icon}</span>
              <span className={`font-bold text-sm ${cat.color}`}>{cat.label}</span>
              <span className="text-xs text-gray-400 font-medium">
                {cat.swGroups.length}종 SW
              </span>
            </div>
            ...
            {cat.swGroups.map((sw, idx) => {
              ...
              return (
                <div key={sw.swName} className={idx > 0 ? "border-t border-gray-100" : ""}>
                  ...
                  >
                    <div className="w-2 h-2 rounded-full bg-gray-300 group-hover:bg-blue-400 transition-colors shrink-0" />
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      {sw.stock > 0 && (
                        <span className="text-xs text-green-700 bg-green-50 border border-green-100 px-2 py-0.5 rounded-full font-semibold">
                          재고 {sw.stock}
                        </span>
                      )}
                      {sw.renewal > 0 && (
                        <span className="text-xs text-orange-700 bg-orange-50 border border-orange-100 px-2 py-0.5 rounded-full font-semibold">
                          갱신필요 {sw.renewal}
                        </span>
                      )}
                      {sw.expired > 0 && (
                  ...
                  {isExpanded && (
                    <div className="bg-gray-50 border-t border-gray-100 px-4 py-3">
                      <div className="text-xs text-gray-500 font-semibold mb-2.5 flex items-center gap-1.5">
                        <span>📋</span> 세부 내역 ({sw.records.length}건)
                      </div>
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="ml-auto flex items-center gap-2">
                                {r.swDetail && (
                                  <span className="text-gray-400">{r.swDetail}</span>
                                )}
                                {r.renewalDate && (
                                ...
                                {r.notionUrl && (
                                  <a
                                    href={r.notionUrl} target="_blank" rel="noopener noreferrer"
...
export default function LicensePanel() {
  ...
  useEffect(() => { setCurrentPage(1); },
    [search, filterType, filterSw, filterStatus, filterCompany, filterDept, filterMacrocat, showExpiringSoon]);

  // ── 드롭다운 옵션 ────────────────────────────────────────────
  const typeOptions    = useMemo(() => ["전체", ...Array.from(new Set(records.map(r => r.licenseType).filter(Boolean)))], [records]);
  const swOptions      = useMemo(() => ["전체", ...Array.from(new Set(records.map(r => r.swCategory).filter(Boolean))).sort((a, b) => a.localeCompare(b, "ko"))], [records]);
  ...
  const filtered = useMemo(() => {
    return records.filter(r => {
      ...
      if (filterMacrocat !== "전체") {
        const { label } = getSwMacroCategory(r.swCategory);
        if (label !== filterMacrocat) return false;
      }
      if (showExpiringSoon) {
  ...
  }, [records, filterType, filterSw, filterStatus, filterCompany, filterDept, filterMacrocat, showExpiringSoon, search]);
  ...
  // ── 통계 ────────────────────────────────────────────────────
  const stats = useMemo(() => {
    const expiring = records.filter(r => { const d = daysLeft(r.renewalDate); return d !== null && d >= 0 && d <= 30; });
    return {
  ...
  const activeFilters = [
    ...
    { label: `SW: ${filterSw}`,       active: filterSw     !== "전체", clear: () => setFilterSw("전체")     },
  ...
  function resetAllFilters() {
    setSearch(""); setFilterType("전체"); setFilterSw("전체");
    setFilterStatus("전체"); setFilterCompany("전체"); setFilterDept("전체");
    setFilterMacrocat("전체"); setShowExpiringSoon(false);
  }

  const thBase = "px-3 py-2.5 text-left text-xs font-semibold text-gray-500 select-none cursor-pointer hover:text-blue-600 whitespace-nowrap";
  const thStatic = "px-3 py-2.5 text-left text-xs font-semibold text-gray-500 whitespace-nowrap";
  ...
  return (
    <div className="fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        {[
          { label: "전체 레코드",   value: stats.total,    color: "border-l-blue-500",   sub: `${filtered.length}건 필터됨` },
          { label: "사용중",        value: stats.using,    color: "border-l-blue-400",   sub: "사용중 + 신규등록" },
          { label: "재고",          value: stats.stock,    color: "border-l-green-500",  sub: "사용 가능 재고" },
          { label: "갱신 임박",     value: stats.expiring, color: "border-l-red-500",    sub: "30일 이내 갱신 필요" },
        ].map(s => (
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-gray-400 font-medium">{filtered.length}건 조회됨</span>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 mb-4 space-y-3">
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            />
            {search && (
              <button onClick={() => setSearch("")} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 text-base leading-none">×</button>
            )}
          </div>
          <button
      ...
      {/* ════════════════════════════════════════════════════
          카테고리별 뷰
      ════════════════════════════════════════════════════ */}
      {viewMode === "category" && <CategoryView records={filtered} />}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className={thBase} onClick={() => toggleSort("swCategory")}>
                    SW <SortIcon col="swCategory" sortKey={sortKey} sortDir={sortDir} />
                  </th>
                  <th className={thBase} onClick={() => toggleSort("licenseType")}>
                    유형 <SortIcon col="licenseType" sortKey={sortKey} sortDir={sortDir} />
                  </th>
                  <th className={thStatic}>버전</th>
                  <th className={thBase} onClick={() => toggleSort("user")}>
                    사용자 <SortIcon col="user" sortKey={sortKey} sortDir={sortDir} />
                  </th>
                  <th className={thBase} onClick={() => toggleSort("department")}>
                    부서 <SortIcon col="department" sortKey={sortKey} sortDir={sortDir} />
                  </th>
                  <th className={thBase} onClick={() => toggleSort("company")}>
                  ...
                  <th className={thBase} onClick={() => toggleSort("renewalDate")}>
                    갱신 필요일 <SortIcon col="renewalDate" sortKey={sortKey} sortDir={sortDir} />
                  </th>
                  <th className={thBase} onClick={() => toggleSort("usageDate")}>
                    사용일자 <SortIcon col="usageDate" sortKey={sortKey} sortDir={sortDir} />
                  </th>
                  <th className={thStatic}>노션</th>
                </tr>
              </thead>
              <tbody>
                {paginated.length === 0 ? (
