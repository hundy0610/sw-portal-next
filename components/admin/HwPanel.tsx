...
function ExcelUploadTab(){
  ...
  const handleFile=async(file:File)=>{
    setPErr("");setRows([]);setResults(null);setSummary(null);sync.resetSync();
  ...
  const doUpload=async(targetRows:ExcelRow[])=>{
    ...
    try{
      ...
      // 신규 지급 이력 기록 + 자산흐름관리 연동 대상 수집 (성공 건만, 엑셀/PC신규등록 공용 후속 체인)
  ...
  const reset=()=>{setRows([]);setFile("");setPErr("");setResults(null);setSummary(null);setProgress(0);sync.resetSync();setShowDupModal(false);setDupItems([]);setCleanRows([]);if(fileRef.current)fileRef.current.value="";};
