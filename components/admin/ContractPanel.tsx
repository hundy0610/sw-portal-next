...
function isExpiringSoon(c: Contract) {
  ...
  return daysUntil(c.endDate) <= 60 && daysUntil(c.endDate) > 0;
}
...
// ── 빈 폼 ─────────────────────────────────────────────────────
const EMPTY_FORM: Omit<Contract, "id" | "status" | "createdAt" | "updatedAt"> = {
  company: "",
  contactName: "",
  contactEmail: "",
  contactPhone: "",
...
export default function ContractPanel() {
  ...
  const AlertBanner = expiringSoon.length > 0 && (
    <div
      className="mb-5 flex items-start gap-3 px-4 py-3 rounded-lg border"
    ...
    >
      ...
      <div>
        ...
        <div className="text-xs mt-1 space-y-0.5" style={{ color: "#974F0C" }}>
          {expiringSoon.map(c => (
            <div key={c.id}>
              · <strong>{c.company}</strong> — {fmt(c.endDate)} 만료 (
              {daysUntil(c.endDate)}일 후)
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="max-w-6xl mx-auto fade-in">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          { label: "진행중 계약", value: `${active.length}건`, icon: "📋", color: "#E9F2FF", tc: "#0052CC" },
          { label: "총 관리 PC", value: `${totalQty.toLocaleString()}대`, icon: "💻", color: "#E3FCEF", tc: "#006644" },
          { label: "월 수익", value: won(monthlyRevenue), icon: "💰", color: "#FFFAE6", tc: "#974F0C" },
          { label: "연 수익", value: won(annualRevenue), icon: "📈", color: "#FFEBE6", tc: "#BF2600" },
        ].map(k => (
          <div
            key={k.label}
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            {(["all", "active", "expired", "pending"] as const).map(f => {
              ...
              return (
                ...
                >
                  ...
                  {f === "active" && ` (${contracts.filter(c => c.status === "active").length})`}
                  {f === "expired" && ` (${contracts.filter(c => c.status === "expired").length})`}
                  {f === "pending" && ` (${contracts.filter(c => c.status === "pending").length})`}
                </button>
              );
            })}
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-400 text-sm">
            {contracts.length === 0 ? "등록된 계약이 없습니다. 계약을 등록해보세요." : "검색 결과가 없습니다."}
          </div>
        ) : (
              ...
              <tbody>
                {filtered.map(c => {
                  ...
                  return (
                    <tr
                      key={c.id}
                      style={expiring ? { background: "#FFFDF0" } : undefined}
                    >
                      <td>
                        <div className="flex items-center gap-2">
                          ...
                          {expiring && (
                            <span className="text-xs px-1.5 py-0.5 rounded font-medium"
                              style={{ background: "#FFE380", color: "#974F0C" }}>
                              만료임박
                            </span>
                          )}
                        </div>
                      ...
                      <td className="whitespace-nowrap">
                        ...
                        {c.status === "active" && (
                          <div className="text-xs text-gray-400">
                            {daysUntil(c.endDate) > 0
                              ? `${daysUntil(c.endDate)}일 남음`
                              : "만료됨"}
                          </div>
                        )}
                      </td>
          ...
          >
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <input className="form-input" placeholder="예) 베어월드 주식회사" value={form.company} onChange={field("company")} />
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <input className="form-input" placeholder="홍길동" value={form.contactName} onChange={field("contactName")} />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">이메일</label>
                  <input className="form-input" placeholder="hong@company.com" value={form.contactEmail} onChange={field("contactEmail")} />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">연락처</label>
                  <input className="form-input" placeholder="010-0000-0000" value={form.contactPhone} onChange={field("contactPhone")} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="date" className="form-input" value={form.startDate} onChange={field("startDate")} />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">
                    계약 종료일 <span className="text-red-500">*</span>
                  </label>
                  <input type="date" className="form-input" value={form.endDate} onChange={field("endDate")} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">계약서 PDF 링크</label>
                  <input
                    className="form-input" placeholder="https://drive.google.com/..."
                    value={form.pdfUrl} onChange={field("pdfUrl")}
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">파일명 (표시용)</label>
                  <input
                    className="form-input" placeholder="계약서_베어월드_2025.pdf"
                    value={form.pdfName} onChange={field("pdfName")}
                  />
                </div>
              </div>
            ...
            <div className="text-center">
              ...
              <div className="flex gap-2 justify-center">
                <button
                  ...
                  className="flex-1 px-4 py-2 text-sm rounded-lg text-gray-600 hover:bg-gray-100 transition-colors font-medium border border-gray-200"
                >
                  취소
                </button>
                <button
